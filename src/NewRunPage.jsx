...
const RunPage = () => {
    ...
    fetch(`/api/runs/${session_id}/${run_id}`, { method: "GET" })
      ...
      .then((response) => {
        ...
        fetch(`/api/classes/${response.class_id}/students`, { method: "GET" })
          .then((res) => res.json())
          .then((studs) => {
            if (studs.filter((s) => s._id == response.student).length) setSelectedStudent(studs.find(s => s._id == response.student));
            setStudents(studs);
            setLoading(false);
          });
      });
  ...
  useEffect(() => {
    if (!map || !data) return;
    if (data.beacons.length || data.rawPositions.length)
		map.fitBounds(featureGroupRef.current.getBounds(), { animate: false });
  }, [map, data]);
  ...
  useEffect(() => {
	...
	console.log("vma", data.avgSpeed, selectedStudent.vma)
        ...
        <div className="run-page-container">
          ...
          <div id="map-wrapper" className={gameMode ? "map-wrapper-dark" : ""} ref={mapWrapperRef}>
            ...
            >
              ...
              <div id="tooltip" ref={tooltipRef}>
                ...
                {hoveredMarker && hoveredMarker.valided && (
                  <>
                    Vitesse: {hoveredMarker.avgSpeed.toFixed(1)} km/h<br />
            ...
            <div id="speed-gauge" className="gauge">
              <Progress
                type="dashboard"
                percent={selectedStudent ? (data.avgSpeed / selectedStudent.vma) * 100 : 0}
                format={() => (
                  <>
			...
			<div id="map-switch">
				<Switch checked={gameMode} onChange={() => {setGameMode(curr => !curr)}} />
			</div>
          </div>
                    ...
                    <ResponsiveContainer width="100%" height="100%">
                      ...
                      >
                        ...
                        {data.beacons
                          .filter((b) => b.valided)
                          .map((b) => (
                            <ReferenceLine x={b.index} stroke="red" ><Label value={b.id} offset={5} position="right" /></ReferenceLine>
                          ))}
                      </LineChart>
                    </ResponsiveContainer>
                  </div>
                  ...
                  >
                    <Row>
                      <Col span={12}>
                        <Row>
                          <Col span={4}>
                            <Avatar src={`https://avatars.dicebear.com/api/bottts/${selectedStudent._id}.svg`}></Avatar>
                          </Col>
                          <Col span={20}>
                      ...
                      <Col span={12}>
                        <Form.Item name="rate" label="" initialValue={data.rating} value>
						  <div className="smiley-rating" value={selectedSmiley}>
							  <img src={smiley1} alt="bad" onMouseDown={() => {setSelectedSmiley(1)}} className={selectedSmiley === 1 ? "selected" : ""}/>
							  <img src={smiley2} alt="bad" onMouseDown={() => {setSelectedSmiley(2)}} className={selectedSmiley === 2 ? "selected" : ""}/>
							  <img src={smiley3} alt="bad" onMouseDown={() => {setSelectedSmiley(3)}} className={selectedSmiley === 3 ? "selected" : ""}/>
							  <img src={smiley4} alt="bad" onMouseDown={() => {setSelectedSmiley(4)}} className={selectedSmiley === 4 ? "selected" : ""}/>
							  <img src={smiley5} alt="bad" onMouseDown={() => {setSelectedSmiley(5)}} className={selectedSmiley === 5 ? "selected" : ""}/>
						  </div>
                        </Form.Item>
                        <Form.Item>
